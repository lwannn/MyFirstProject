...
import com.ytj.project_login.MainActivity;
...
//    private TextView mTextView;
    ...
    public void onCreate(Context context, LayoutInflater inflater) {
        mRootView = inflater.inflate(R.layout.fragment_message, new FrameLayout(context), true);

        this.context = context;
        titleConfiguration = (MainActivity_w) context;
        netServiceIntent = new Intent(context, NetService.class);
        Log.e("ttttttt", "onStartCommand: " + getCurProcessName(context));
        initData();
        initView();
        initEvent();
    }
    ...
    private void initData() {
        mUsername = ((Activity) context).getIntent().getStringExtra("username");
                ...
                .execute(new BitmapCallback() {
                    ...
                    public void onResponse(Bitmap bitmap) {
                        if (bitmap != null)//在获取的图片不为空的情况下
                            ...
                            titleConfiguration.setTitleImage(bitmap);
                        //TODO 将图片保存到本地
                    }
                });
    }
        ...
        OkHttpUtils
                ...
                .execute(new StringCallback() {
                    ...
                    public void onResponse(String s) {
                        ...
                        SharePreferencesUtil.setParam(context, "caseId", sb_id.toString());
                        SharePreferencesUtil.setParam(context, "caseName", sb_name.toString());
                        //通过组的id获取组的成员的详细信息（并且保存到数据库中）
                        int deptid = userRoot.getDat().getDeptid();
                        getUsersInfo(deptid);

                        //将该用户的id存到变量中
                        DetailActivity.MINE_ID = userRoot.getDat().getId();
                        MINE_ID = userRoot.getDat().getId();
                        SharePreferencesUtil.setParam(context, "userId", Integer.toString(MINE_ID));
                        Intent warnServiceIntent = new Intent(context, WarnService.class);
                        Log.i("WarnService", "onResponse: yesyesyes");
                        context.startService(warnServiceIntent);
                        //将该用户的name存到static变量中
                        DetailActivity.MINE_NAME = userRoot.getDat().getAlias();
                        //将该用户的tel存到static变量中
                        DetailActivity.MINE_TEL = userRoot.getDat().getTel();

                        //设置title
                        titleConfiguration.setTitleText(userRoot.getDat().getAlias());
...
//                        mTextView.setText(userRoot.getDat().getAlias());
    ...
    void refershUI() {
        ...
        OkHttpUtils
                .get()
                .url(getConnectionUrl(String.valueOf(MINE_ID)))
                .build()
                .execute(new StringCallback() {
                    @Override
                    public void onError(Call call, Exception e) {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        switch (keyCode) {
            case KeyEvent.KEYCODE_BACK:
                ...
                context.startService(netServiceIntent);
                break;
    ...
    public interface TitleConfiguration{
