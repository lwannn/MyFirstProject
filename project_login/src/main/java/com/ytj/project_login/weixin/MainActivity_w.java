...
public class MainActivity_w extends Activity implements DetailFragment.TitleConfiguration {
    ...
    MineInfoFragment mineInfoFragment;
        ...
        public void onClick(View v) {
            switch ((int) v.getTag()) {
                case 0:
                    ...
                    fragmentTransaction.replace(R.id.lay_fragment, getDetail_Fragment());
                    fragmentTransaction.commit();
                    break;
                case 1:
                    fragmentTransaction = fragmentManager.beginTransaction();
                    fragmentTransaction.replace(R.id.lay_fragment, getCaseFragment());
                    fragmentTransaction.commit();
                    break;
                case 2:
                    fragmentTransaction = fragmentManager.beginTransaction();
                    fragmentTransaction.replace(R.id.lay_fragment, getMineInfoFragment());
                    fragmentTransaction.commit();
                    break;
            }
        }
    ...
    private CaseFragment getCaseFragment() {
        CaseFragment _caseFragment;
        if (caseFragment == null) {
            caseFragment = new CaseFragment();
        }
        _caseFragment = caseFragment;
        return _caseFragment;
    }

    private Detail_Fragment getDetail_Fragment() {
        Detail_Fragment _detail_fragment ;
        if (detail_fragment == null) {
            detail_fragment = Detail_Fragment.newInstance();
        }
        _detail_fragment = detail_fragment;
        return _detail_fragment;
    }

    private MineInfoFragment getMineInfoFragment() {
        MineInfoFragment _mineInfoFragment;
        if (mineInfoFragment == null) {
            mineInfoFragment = new MineInfoFragment();
        }
        _mineInfoFragment = mineInfoFragment;
        return _mineInfoFragment;
    }
